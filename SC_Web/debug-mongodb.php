<?php
/**
 * MongoDB Extension Debug Script
 * This script helps diagnose MongoDB extension issues
 */
...
echo "<h1>MongoDB Extension Debug</h1>";
...
echo "<h2>9. Manual Extension Loading Test</h2>";
if (function_exists('dl')) {
    echo "dl() function is available<br>";
    // Try to load MongoDB extension manually
    $ext_dirs = glob('/usr/local/lib/php/extensions/*/mongodb.so');
    if (!empty($ext_dirs)) {
        echo "Found MongoDB extension files: " . implode(', ', $ext_dirs) . "<br>";
    ...
    echo "❌ dl() function not available (disabled in production)<br>";
}

echo "<h2>10. Full PHP Info</h2>";
