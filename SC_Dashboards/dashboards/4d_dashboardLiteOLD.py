    ...
    try:
            ...
            print(f"  is_3d_volume: {is_3d_volume}")
                ...
                try:
                    ...
                    if box_annotation_2 is not None:
                        ...
                        try:
                            # Try to extract numeric values from box_annotation_2
                            # Node objects might need special handling - try multiple approaches
                            def extract_value(attr_value):
                                """Extract numeric value from attribute, handling Node objects."""
                                if attr_value is None:
                                    return None
                                # If it's already a number, return it
                                if isinstance(attr_value, (int, float)):
                                    return float(attr_value)
                                # Try to get value property if it's a Node
                            ...
                            left_val = extract_value(box_annotation_2.left)
                            right_val = extract_value(box_annotation_2.right)
                            bottom_val = extract_value(box_annotation_2.bottom)
                            top_val = extract_value(box_annotation_2.top)
                            
                            print(f"  🔍 Extracted values: left={left_val}, right={right_val}, bottom={bottom_val}, top={top_val}")
                            
                            if (left_val is not None and right_val is not None and 
                                bottom_val is not None and top_val is not None and
                                not (np.isnan(left_val) or np.isnan(right_val) or np.isnan(bottom_val) or np.isnan(top_val))):
                                ...
                                print(f"  ⚠️ box_annotation_2 values are None or NaN, falling back to rect2")
        ...
        def compute_plot2_from_plot3():
            ...
            def do_computation_2a():
                ...
                try:
                    # Helper function to extract numeric values (same as for box_annotation_2)
                    def extract_value(attr_value):
                        """Extract numeric value from attribute, handling Node objects."""
                        if attr_value is None:
                            return None
                    ...
                    if box_annotation_3 is not None:
                        try:
                            ...
                            left_val_3 = extract_value(box_annotation_3.left)
                            right_val_3 = extract_value(box_annotation_3.right)
                            bottom_val_3 = extract_value(box_annotation_3.bottom)
                            top_val_3 = extract_value(box_annotation_3.top)
                            
                            print(f"  🔍 Extracted box_annotation_3 values: left={left_val_3}, right={right_val_3}, bottom={bottom_val_3}, top={top_val_3}")
                            ...
                            if (left_val_3 is not None and right_val_3 is not None and 
                                ...
                                print(f"  ✅ Using box_annotation_3 coordinates: x=[{x1_coord}, {x2_coord}], y=[{y1_coord}, {y2_coord}]")
                        except Exception as e:
                    ...
                    if not use_box_annotation_3:
                        ...
                        print(f"  ✅ Using rect3 coordinates: x=[{x1_coord}, {x2_coord}], y=[{y1_coord}, {y2_coord}]")
                    ...
                    print(f"  📊 Selection size: X={x2-x1}/{volume.shape[0]} ({((x2-x1)/volume.shape[0]*100):.1f}%), Y={y2-y1}/{volume.shape[1]} ({((y2-y1)/volume.shape[1]*100):.1f}%)")
                    ...
                    if is_3d_volume:
                        ...
                        # Trigger update by accessing the data property (Bokeh will detect the change)
                        _ = source2.data
                        plot2.x_range.start = float(np.min(x_coords_1d))
                        plot2.x_range.end = float(np.max(x_coords_1d))
